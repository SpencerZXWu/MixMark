...
(function () {
   ...
   * 首启示例文档（欢迎文档）。
   *
   * 正文与标题都在 core/welcome.js —— 它挺长，而且是双语的，摆在这里会把
   * 这个文件淹掉。
   *
   * 顶栏还会用它做一件事：这份示例的标题跟最左边的品牌名是同一句话，
   * 摆在一起就是说了两遍，所以标题位留空（见 isWelcomeTitle）。
   */
  function welcomeContent() {
    return MM.welcome.content();
  }
  ...
  MM.docs = {
    ...
    /** 顶栏用它判断“标题位该不该留空”，见 welcomeContent 的注释 */
    isWelcomeTitle: function (title) {
      return MM.welcome.isTitle(title);
    }
  };
})();
