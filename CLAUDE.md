...
45. **反向修改的三条命（改 preview/live-edit.js 之前先读）**
    预览区直接编辑，每一环都有个不显眼但致命的细节：

    - **编辑期间必须挂起重新渲染。** 每敲一下都写回源码，而源码变化会触发
      重新渲染 —— 那一重建就把正在编辑的元素连同焦点整个换掉，一个字也敲不
      进去。`MM.preview.setHold(true)` 干的就是这个，解除时对齐一版。
      **挂起必须配一个兜底解除**：`focusout` 在无前台焦点的环境里不派发，
      只靠它的话 hold 会永久卡住，表现是「预览再也不跟着源码更新」。所以还有
      一条文档级 `pointerdown`，点到预览外面就落地并解除。
    - **块的行号要手动平移。** 写回会改变行数，而其他块的 `data-line` 是上一
      次渲染算好的。`shiftBlocks()` 按 delta 把它们推一遍，否则紧接着编辑下
      一块就会写到别的行上。
    - **异步渲染的元素不能被当成「还在」。** `relink()` 一开始只认
      `.mm-mermaid`，而 mermaid 是异步画的 —— 重渲染刚结束时 DOM 里还只有
      `pre > code.language-mermaid`，于是它找不着块，`ctx.block` 一直抱着已经
      脱离文档的旧元素，行号全是过期的，下一次编辑改到别的段落上去。
      判据要**两种形态都认**。同理 `flush()` 入口挡一道 `isConnected`。
    - `MM.mdOut` 是「整块重写」：要准就得把 DOM 结构译回 Markdown，而只要译
      一次，那一块内部的手工排版（表格竖线对齐之类）就回不来了。块与块之间的
      空行、缩进、注释不受影响。

46. **`data-line-end` 不能拿「换行数 - 1」推**
    `token.raw` **尾部带不带换行并不确定**（最后一块、某些块类型就不带），
    所以「换行数 - 1」在两种情况里必然有一种算错一行。错了以后：代码块会丢掉
    闭合围栏（于是它再也不是 ```mermaid，反向修改和图表都认不出它），
    任务列表会丢掉最后一条 —— 而且只在特定文档末尾才显形。
    正确做法是**先剥掉尾部换行、再数里面剩下几个换行**。

47. **两个控件表达同一件事，一定会打架**
    图表连线一开始同时有「箭头」和「线型」两个下拉框。线型那边按一套规则去改
    箭头里的字符（`-->` → `-.>`），而类图的虚线是 `..>`、流图的才是 `-.->` ——
    生出来的写法类图不认，整张图直接报错。
    mermaid 本来就把线型写在箭头里（`-->` / `..>` / `-.->` / `==>` / `-->>`），
    所以删掉线型那个框，只留箭头，在选项文字上标明哪个是虚线。
    **判据：如果两个控件的取值能互相推导，它们就不该共存。**

48. **`detectType` 的返回值要去查表，就别自作主张换大小写**
    它给状态图特例返回了 camelCase 的 `stateDiagram-v2`，而 `STRUCTURED` 表的
    键是小写的 `statediagram-v2` —— 查不到，整个状态图静悄悄退化成「只有原始
    行」，还不报错。类型名只在**一个地方**归一到展示用写法。

49. **形状 / 操作符这类「按表匹配」，长的必须排在前面**
    流程图节点形状表里 `(` 排在 `((` 前面，`E((圆形))` 就被 `(` 抢先命中判成
    「圆角」。改法不是手工调数组顺序（下次加一项又会被破坏），而是**按包裹符
    长度排一遍再遍历**。
    同一类问题还有连线箭头：原先拿正则在行里找「第一个像箭头的东西」，碰到类图
    的 `<|--`（继承）只认出了中间的 `--`，`<|` 留在左端点上 —— 左端点当场解析
    失败，整条继承关系降级成「原始行」。换成枚举已知写法，位置相同时取更长的。

50. **元素名不只用 `\w`**
    状态图里的状态名常常直接写中文（`待机 --> 运行`），而 JS 的 `\w` 只管
    ASCII —— 一个中文状态都解析不出来。用「不是空白、也不是各种括号分隔符」
    才是对节点名的正确描述。

51. **一份「东西变了」的通知，必须从那个东西真正变的地方发出**
    `preview:rendered` 一开始只在 `schedule()`（防抖那一头）里发，后来
    `MM.preview.renderNow` 换成了另一条不发的路径也没人发现 —— 因为两类
    调用者都只是「偶尔经过」schedule。结果：切文档、切主题、解除反向修改
    的挂起……所有这些渲染都不发事件，行号索引不重建、可编辑属性也不重挂，
    表现是「重渲染之后预览区点不进去」，而单看代码根本看不出问题。

    改法是把 emit 收到 `renderNow` 末尾（唯一的真正出口），并删掉别处的
    重复 emit。**凡是「多个入口、一个真相」的地方，检查一下通知是不是
    也从多个地方发、或者漏发。**

52. **mermaid 的 `render()` 不可重入，必须串行**
    它要往 body 里插临时容器、量完文字宽度再搬走，两个渲染叠在一起会抢
    那套全局状态 —— promise 既不 resolve 也不 reject，临时容器永远留在
    body 里，图表块退回原始代码。反向修改里编辑图表时重渲染很频，这条路
    必踩。用一个 promise 队列把它们串起来，一次只跑一个。

53. **`getElementById(id)` 当「清理残留」的兜底会误伤**
    mermaid 会把同一个 id 也写在它生成的 `<svg>` 上。清理临时容器的
    `getElementById('d' + id) || getElementById(id)` 里，后半截正好命中
    刚插进预览的那张图 —— 现象是「容器在、里面空的」，比不渲染还难查。
    清理目标要写死，不要用 id 前缀去「顺手兜一下」。

54. **预览区反向修改的四条保命规则**
    除上面 45-47 条外，这轮又补了两条：
    - `activate()` 里要**顺手确保这一块可编辑**。正常由渲染后统一挂
      `contenteditable`，但只要有**任何**一条渲染路径漏了通知，点进去就
      一个字也打不了，且看起来很像「改了又变回去」。宁可每次点的时候
      多写两个属性。
    - 面板上的文本框一律**防抖**再提交。一字符一提交 = 一字符一整片
      预览重渲染，视觉上就是一直闪，而 mermaid 还会在重渲染期间短暂
      退回代码块。下拉框 / 增删这类「点一下就是一下」的操作不防抖。

55. **回写不能把「渲染后的 DOM」当成 Markdown 的等价物**
    这是反向修改里最大的一个坑，而且是安静的：预览里很多东西早就不是
    原文了，照着它反推 Markdown 会静默地改掉用户没碰过的内容。已踩到的：

    - **公式**。`$E=mc^2$` 在 DOM 里是 KaTeX 的 HTML + MathML 两套渲染，
      拉平就是 `E=mc2E = mc^2E=mc2`。原始 TeX 要去 MathML 的
      `<annotation encoding="application/x-tex">` 里拿 —— KaTeX 存那份
      就是为了「复制公式能粘回去」。
    - **图片地址**。拖拽引用会把裸文件名换成 `blob:` URL（只在这一会话
      有效），写进文档就永久坏了。换之前先记在 `data-md-src` 上。
    - **换行**。开了 `breaks` 时单个换行就渲染成 `<br>`，回写一律写
      「两个空格 + 换行」的话，每编辑一次就给每行尾巴添两个空格。
    - **表格对齐**。`:---:` 这些写在分隔行里，渲染后变成 `th` 上的
      align / style；不读回来，改一次表格就把三列对齐全部拍平。

    守这门规矩的办法是两套自动检查（都是全量扫描，不靠抽样本）：
    1. **往返一致性**：把每一块的 DOM 序列化回去，必须与源码**一字不差**。
    2. **编辑保真**：在每块插一个标记再回写，去掉标记后必须与原文一字不差。

    第 2 条才是「只改用户改的那一处」的真正证明 —— 第 1 条过了不代表第 2 条
    过：中间还可能插错位置。新增块类型时两套都要跑。

56. **「渲染出来的文字」和「源码里的文字」不是一回事，所以公式要设成不可编辑**
    回写公式时 TeX 是从 annotation 取的，所以往渲染结果里敲字根本落不到
    源码上 —— 敲了半天什么也没发生，比敲不进去更让人困惑。
    把公式标成 `contenteditable="false"`：光标进不去，整块选中删掉就是
    「删掉这条公式」，语义清楚。
    一般规律：**凡是「显示出来的东西无法反映回源码」的元素，都不该让用户
    直接编辑它。**
